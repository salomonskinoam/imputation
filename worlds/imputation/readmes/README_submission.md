...
record under [`tasks/`](tasks/) are produced by `tools/band_report_impute.py` (which fills the
...
## Per-task records + the analysis seam

Two files per task, kept in sync by the invariant validator:
- **record** (GENERATED) under [`tasks/`](tasks/), linked from each row's `[record]`. Do not hand-edit.
- **strategy analysis** (HUMAN-owned) at `../analysis/<task>/STRATEGY.md`, linked from each row's
  `[analysis]`. This is where the 5-solution comparison (what drove weak->strong; luck vs skill) lives. It
  is updated independently: editing a STRATEGY.md never re-touches this table or the records (that is the
  point). `--emit` writes a `[PENDING]` stub if one is missing and never overwrites a filled one. See the
  SDK `horizon-band-verdict` skill + `sdk/methodology/noise_floor.md` §15b.
